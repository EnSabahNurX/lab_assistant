...
function Selecionar-ArquivoOuPasta {
    ...
    if ($row -ne $null) {
        if ([string]::IsNullOrEmpty($row.Nome)) {
            ...
            $opcao = [System.Windows.MessageBox]::Show("Deseja selecionar um arquivo ou um diretório?", "Selecionar", [System.Windows.MessageBoxButton]::YesNoCancel, [System.Windows.MessageBoxImage]::Question)
            if ($opcao -eq [System.Windows.MessageBoxResult]::Yes) {
                ...
                $caminho = SelecionarArquivoOuPasta "Selecionar diretório" $false
            }
